<script setup lang="tsx">
  ...
  const getNavIcon = (item: RouteMeta | undefined) => {
    ...
    return (
      <ElIcon size="0.5rem" style="margin-right: 8px; padding: 0 1rem 0 1.5rem;">
        {typeof item.icon === 'string' ? <img src={`./${item.icon}.png`} width="20" height="20" /> : h(item?.icon as Component)}
      </ElIcon>
    )
  }
  ...
  const MenuItemLink = (props: { route: RouteRecordRaw; url: string }, { slots }: { slots: Slots }) => {
    if (props.route.meta?.external) {
      return (
        <a href={props.route.redirect as string} target="_blank" ref="noopener noreferrer">
          {slots.default?.()}
        </a>
      )
    }
  ...
  const MenuItemNav = (props: { route: RouteRecordRaw; basePath: string }) => {
    ...
    if (props.route.meta?.alwaysShow) {
      return subMenuTemplate(props.route)
    }
  ...
  const TheSideBar = () => (
    <Scrollbar>
      ...
      >
        {routesList.value.map((route, index) => (
          <MenuItemNav key={index} route={route} basePath={route.path}></MenuItemNav>
        ))}
      </ElMenu>
    </Scrollbar>
  )
